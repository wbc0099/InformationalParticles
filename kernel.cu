...
void getInput()
{
    ...
    while (std::getline(InputFile, line))
    {
        ...
        case 5:
            ...
            break;
        case 6:
            iss >> PM.maxParticlePerCell;
            break;
        case 7:
            iss >> PM.rd;
            break;
        case 8:
            iss >> PM.miniInstanceBetweenParticle;
            break;
        case 9:
            iss >> PM.r0;
            break;
        ...
        case 22:
            ...
            break;
        case 23:
            iss >> PM.forceCoefficient;
            break;
        case 24:
            iss >> PM.rNeighborList;
            break;
        case 25:
            iss >> PM.kBTChangeMode;
            break;
        case 26:
            iss >> PM.kBTChangePM0;
            break;
        case 27:
            iss >> PM.visionConeXLen;
            break;
        default:
            break; // 超过预期行数时不处理
        }
...
__global__ void getAroundCellParticleId(Particle PT, Parameter PM)
{
    ...
    for (int x = -1; x <= 1; x++)
    {
        for (int y = -1; y <= 1; y++)
        {
            ...
            {
                if (PT.cellList[cellAroundId * PM.maxParticlePerCell + i] == id)
                    continue;
                int ifNeighbor = getNeighborListTry(PT.x[id], PT.y[id], PT.x[PT.cellList[cellAroundId * PM.maxParticlePerCell + i]], PT.y[PT.cellList[cellAroundId * PM.maxParticlePerCell + i]], PM);
                if (ifNeighbor)
                {
...
__device__ int getNeighborListTry(real x0, real y0, real x1, real y1, Parameter PM)
{
    ...
    if (dr2 < PM.rNeighborList * PM.rNeighborList)
        return 1;
    else
        return 0;
}
...
__device__ void updateKBT(Particle PT, Parameter PM, int id)
{
    ...
    {
        PT.kBT[id] = sin(PT.aroundNum[id] * Pi / 300 + Pi / 2) + 1;
    }
}
...
__global__ void checkUpdate(Particle PT, Parameter PM)
{
    ...
    dx = sign01(0.5 * PM.boxX - dx) * dx + sign01(dx - 0.5 * PM.boxX) * (PM.boxX - dx);
    dy = sign01(0.5 * PM.boxY - dy) * dy + sign01(dy - 0.5 * PM.boxY) * (PM.boxY - dy);
    if ((dx * dx + dy * dy) > PM.rOutUpdateList * PM.rOutUpdateList)
        atomicExch(&updateListFlag, 1);
}
